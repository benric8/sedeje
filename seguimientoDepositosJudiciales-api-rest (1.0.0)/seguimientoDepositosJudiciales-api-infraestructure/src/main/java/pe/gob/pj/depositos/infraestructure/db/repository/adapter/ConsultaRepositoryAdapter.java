...
public class ConsultaRepositoryAdapter implements ConsultaRepositoryPort, Serializable {
	...
	public List<DepositoJudicial> consultarDeposito(String cuo, String numeroExpediente) throws Exception { 
		...
		query.getResultStream().forEach(depJudicial -> {
			if (depJudicial != null && depJudicial.getCDepositoJ() != null) {
				...
				/*
				 * try { depositoJudicial.setOrdenesPago(consultarOrdenesPago(cuo,
				 * depJudicial.getCDepositoJ())); }catch(Exception e){
				 * 
				 * }
				 */
				...
				if(depJudicial.getCEstado().equals(ProjectConstants.ESTADO_DJ_D)) {
					...
					if (!depJudicial.getOrdenesPago().isEmpty()) {
						...
						List<MovDepOrdenPago> ordenesPagoPorDeposito = depJudicial.getOrdenesPago().stream().collect(Collectors.toList());
						...
						Collections.sort(ordenesPagoPorDeposito,Comparator.comparing(MovDepOrdenPago::getFAutorizaPri));
						log.info("AGREGAMOS LAS ORDENES COMO ESTADOS DE DEPOSITO");
						ordenesPagoPorDeposito.forEach(ordenPago->{
						...
						depositoJudicial.setOrdenesPago(ordenesPago);		
						if(!ordenesPagoPorDeposito.isEmpty()) {
							...
							if (depJudicial.getCEstado().equals(ProjectConstants.ESTADO_DJ_C)|| depJudicial.getNSaldo()==0) {
								depositosEstado.add(new DepositoEstado(ProjectConstants.ESTADO_DJ_C, ProjectConstants.DESCRIPCION_ESTADO_DJ_C,
										ProjectUtils.convertDateToString(ordenesPagoPorDeposito.get(ordenesPagoPorDeposito.size()== 0 ? 0 :ordenesPagoPorDeposito.size()-1).getFCobroBn(),
												ProjectConstants.FORMATO_FECHA_DD_MM_YYYY_HH_MM), "A","1")); 
							}else { 
								/*
								 * depositosEstado.add(new
								 * DepositoEstado(ProjectConstants.ESTADO_DJ_Q,ProjectConstants.
								 * DESCRIPCION_ESTADO_DJ_Q,
								 * ProjectUtils.convertDateToString(ordenesPagoCobradas.get(ordenesPagoCobradas.
								 * size()== 0 ? 0 :ordenesPagoCobradas.size()-1).getFCobroBn(),
		...
		try { 
			...
			query.getResultStream().forEach(ordenPago -> {
				if (ordenPago != null && ordenPago.getCDepositoJ() != null) {
					...
					ordPago.setCOrdenPago(ordenPago.getCDepositoJ()+"-"+ordenPago.getCOrdenPago());
					ordPago.setFechaEndoso(ProjectUtils.convertDateToString(ordenPago.getFAutorizaSec(),ProjectConstants.FORMATO_FECHA_DD_MM_YYYY_HH_MM));					
					ordPago.setFechaCobro(ordenPago.getFCobroBn()!=null?ProjectUtils.convertDateToString(ordenPago.getFCobroBn(),ProjectConstants.FORMATO_FECHA_DD_MM_YYYY_HH_MM)
							:ordenPago.getCEstado().equals(ProjectConstants.ESTADO_OP_X)?ProjectConstants.DESCRIPCION_ESTADO_OP_A: ProjectConstants.DESCRIPCION_ESTADO_OP_NC);				
					ordenesPago.add(ordPago);
				}
			});
	...
	@Override
	public PaginationDto consultarDeposito(String cuo, String numeroExpediente, Integer page, Integer rows)
			throws Exception {
		List<DepositoJudicial> depositosJudiciales = new ArrayList<DepositoJudicial>();
		...
		try { 
			if(page == 0) {
				...
				StringBuilder countQuery= new StringBuilder("SELECT COUNT (DISTINCT mdj.cDepositoJ) as total FROM MovDepositoJudicial mdj");
				countQuery.append(" where mdj.xNumExp=: " +MovDepositoJudicial.DJ_EXPEDIENTE);
				TypedQuery<Long> queryT = this.sfSij.getCurrentSession().createQuery(countQuery.toString(), Long.class);
				queryT.setParameter(MovDepositoJudicial.DJ_EXPEDIENTE, numeroExpediente);
				Long total = queryT.getSingleResult();	
				BigDecimal ultimaPagina = new BigDecimal(Double.toString(total/rows));
				ultimaPagina = ultimaPagina.setScale(0, RoundingMode.CEILING);
				log.info("{} Total de paginas [{}]", cuo, ultimaPagina);
				paginate.setTotalRecords(total.intValue());
				paginate.setCurrentPage(page);
				paginate.setRows(rows);
				paginate.setStartPage(0);
				paginate.setFinalPage(ultimaPagina.intValue());
				
				log.info("{} Total registros encontrados [{}]", cuo, total);
			} else {
				paginate.setCurrentPage(page);
				paginate.setRows(rows);
				paginate.setStartPage(0);
			...
			paginate.setLista(depositosJudiciales);
		} catch (SQLGrammarException | IllegalArgumentException |
				ConstraintViolationException | DataIntegrityViolationException e) { throw new
			ErrorDaoException(ProjectConstants.C_E015,
					ProjectConstants.X_ERROR +
